...
function exportSalesToExcel(orders) {
  const data = orders.map((order) => ({
    ...
    "Parsed By": order.parsedBy || "",
  }))
  const worksheet = XLSX.utils.json_to_sheet(data)
  const workbook = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(workbook, worksheet, "Sales")
  XLSX.writeFile(workbook, `SellerBot-Sales-${new Date().toLocaleDateString("en-GB").replace(/\//g, "-")}.xlsx`)
}
function SalesTable({ orders, onDelete, onInvoice, onView }) {
  const cellClass = "px-4 py-3 align-middle whitespace-nowrap"

  return <div className="overflow-x-auto rounded-lg border bg-white"><table className="w-full min-w-[1120px] table-auto text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr>{["Order #", "Date", "Customer", "Phone", "Zone", "Items", "Grand Total", "Cost", "Profit", "Payment Type", "Payment Status", "Actions"].map((h) => <th key={h} className="px-4 py-3 whitespace-nowrap">{h}</th>)}</tr></thead><tbody>{orders.map((o) => <tr key={o.id} className="border-t hover:bg-slate-50"><td className={`${cellClass} font-semibold`}>{o.orderNumber}</td><td className={cellClass}>{getOrderDateValue(o).toLocaleDateString("en-GB")}</td><td className="px-4 py-3 align-middle"><span className="block max-w-36 truncate" title={o.customerName || ""}>{o.customerName}</span></td><td className={cellClass}>{o.phone}</td><td className={cellClass}>{o.zone}</td><td className={cellClass}>{o.products?.length || 0}</td><td className={cellClass}>à§³{o.grandTotal || o.grossRevenue || 0}</td><td className={cellClass}>à§³{o.totalCost || 0}</td><td className={cellClass}>à§³{o.grossProfit || 0}</td><td className={cellClass}><PaymentTypeBadge type={o.paymentType} /></td><td className={cellClass}><StatusBadge status={o.paymentStatus || o.productPaymentStatus} /></td><td className={cellClass}><div className="flex items-center gap-3"><Action icon={Eye} label="View" onClick={() => onView(o)} /><Action icon={Printer} label="Invoice" onClick={() => onInvoice(o)} /><Action icon={Trash2} label="Delete" danger onClick={() => onDelete(o)} /></div></td></tr>)}</tbody></table></div>
}
function OrderModal({ order, onClose, onPDF, onPrint, onPaid }) { return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 px-4 py-6"><section className="mx-auto max-w-3xl rounded-lg bg-white p-5"><div className="mb-4 flex justify-between"><h3 className="text-xl font-semibold">Order Details</h3><button onClick={onClose}><X className="h-5 w-5" /></button></div><div className="space-y-3 text-sm"><p><b>Customer:</b> {order.customerName} / {order.phone}</p><p className="whitespace-pre-wrap"><b>Address:</b> {order.address}</p><p><b>Payment Type:</b> {paymentTypeLabel(order.paymentType)}</p><PaymentBreakdown order={order} /><div className="flex flex-wrap items-center gap-2"><b>Parsed by:</b><ChatTypeBadge order={order} /></div><p><b>Notes:</b> {order.notes || ""}</p>{(order.products || []).map((p, i) => <p key={i} className="rounded bg-slate-50 p-2">{p.productName} x {p.quantity} Â· à§³{p.totalPrice}</p>)}</div><div className="mt-5 flex flex-wrap gap-3"><button className="btn-primary" onClick={() => onPDF(order)}>Download Invoice PDF</button><button className="btn-outline" onClick={() => onPrint(order)}><Printer className="mr-2 inline h-4 w-4" />Print Invoice</button>{(order.paymentStatus || order.productPaymentStatus) === "Unpaid" && <button className="btn-outline" onClick={() => onPaid(order)}>Mark as Paid</button>}<button className="btn-outline" onClick={onClose}>Close</button></div></section></div> }
function ChatTypeBadge({ order }) { const ai = order.chatType === "unstructured" || order.parsedBy === "gemini"; return <span className={`rounded-full px-2 py-1 text-xs font-semibold ${ai ? "bg-blue-50 text-blue-800" : "bg-emerald-50 text-emerald-800"}`}>{ai ? "ðŸ¤– AI Parsed" : "ðŸ“‹ Structured"}</span> }
function PaymentBreakdown({ order }) { if (order.paymentType === "delivery_only_online") return <div className="rounded bg-slate-50 p-3"><p>Online Payment: à§³{order.onlineAmount || order.deliveryCharge} via {order.deliveryPaymentMethod} (TXN: {order.deliveryTransactionId || ""}) {order.deliveryPaymentStatus === "Paid" ? "Paid" : "Pending"}</p><p>On Delivery: à§³{order.codAmount || order.subtotal} Cash Pending</p></div>; if (order.paymentType === "full_online") return <p className="rounded bg-slate-50 p-3">Online Payment: à§³{order.onlineAmount || order.grandTotal} via {order.productPaymentMethod} (TXN: {order.productTransactionId || ""})</p>; return <p className="rounded bg-slate-50 p-3">On Delivery: à§³{order.codAmount || order.grandTotal} Cash Pending</p> }
