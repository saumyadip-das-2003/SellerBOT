...
import { getCustomerInsights } from "../utils/graphDB.js"
...
function Dashboard() {
  ...
  useEffect(() => {
    if (!currentUser?.uid) return undefined
    let mounted = true
    getCustomerInsights(currentUser.uid).then((graphData) => {
      if (!mounted || !graphData) return
      setGraphInsights(graphData.insights || null)
      setGraphPairs(graphData.frequentPairs || [])
    })
    return () => {
      mounted = false
    }
