...
import { getCustomerInsights, getFrequentlyBoughtPairs } from "../utils/graphDB.js"
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    let mounted = true
    Promise.all([
      getCustomerInsights(currentUser.uid),
      getFrequentlyBoughtPairs(currentUser.uid, 5),
    ]).then(([insights, pairs]) => {
      if (!mounted) return
      if (insights) setGraphInsights(insights)
      setGraphPairs(pairs || [])
    })
    return () => {
      mounted = false
